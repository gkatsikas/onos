...
/*
 ONOS GUI -- Service Chains' Info View Module
 */
...
(function () {
    ...
    // constants
    var pName = 'scinfo-details-panel',
        panelWidth = 750,
        topPdg = 80,
        propOrder = [
            'id', 'nfs', 'scope', 'dispatcher',
            'ingress', 'processing', 'egress', 'cpus', 'nics'
        ],
        friendlyProps = [
            'Service Chain ID', 'Network Functions', 'Scope',
            'Dispatcher', 'Ingress Points', 'Processing Points',
            'Egress Points', 'CPU Cores', 'NICs'
        ];
    ...
    function populateTop(details) {
        var propsBody = top.select('.scinfo-props table').append('tbody');

        top.select('.scinfo-title-1').text(details.id);
        // top.select('.scinfo-title-2').text(details.prop);
    ...
    angular.module('ovScinfo', [])
        .controller('OvScinfoCtrl',
            ['$log', '$scope',
        ...
        function (_$log_, _$scope_, _fs_, _ks_, _ps_, _is_, tbs) {
            ...
            ks.gestureNotes([
                ['click row', 'Select / deselect scinfo property'],
                ['scroll down', 'See more scinfo'],
            ]);
            ...
            $log.log('OvScinfoCtrl has been created');
        }])

        .directive('scinfoDetailsPanel',
